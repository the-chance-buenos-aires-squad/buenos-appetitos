...
class RecipeParser {
    fun parseRecipes(records: List<List<String>>): List<Recipe> =
        records.filter { it.size >= 12 }.mapNotNull { fields ->
            runCatching { toRecipe(fields) }
                .onFailure {
    ...
    private fun parseTags(input: String): List<String> = runCatching { parseList(input) }.getOrDefault(emptyList())

    private fun parseSteps(input: String): List<String> = runCatching { parseList(input) }.getOrDefault(emptyList())

    private fun parseIngredients(input: String): List<String> =
        runCatching { parseList(input) }.getOrDefault(emptyList())

    private fun parseNutrition(input: String): Nutrition = runCatching {
        parseList(input).mapNotNull { str ->
            str.toDoubleOrNull()
        }.let { values ->
            toNutrition(values)
        }
    ...
    private fun toNutrition(values: List<Double>): Nutrition {
        return Nutrition(
    ...
    private fun toRecipe(fields: List<String>): Recipe {
        return Recipe(
